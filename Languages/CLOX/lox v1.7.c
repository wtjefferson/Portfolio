...
LoxFunction* loxClassGetMethod(LoxInstance* instance, const char* methodName) {
...
void executeClassDecl(ClassDecl* classDecl, Environment* env) {
    // Create a new LoxClass object
    LoxClass* klass = newLoxClass(classDecl->name.lexeme);

    // Iterate over methods and add them to the class
    for (int i = 0; i < classDecl->methods->count; i++) {
        Method* method = classDecl->methods->items[i];
        LoxFunction* function = newLoxFunction(method->function, env, false);  // false for isInitializer
        loxClassAddMethod(klass, method->name.lexeme, function);
    }
